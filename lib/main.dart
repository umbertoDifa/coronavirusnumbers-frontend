...
//    _filtered_corona_data = fetchCoronaData();
...
//    _getCurrentLocation();
  ...
  Expanded _build_bottom_list(BuildContext context, List<CoronaData> corona_data) {
      ...
      itemBuilder: (context, i) {
        return ListTile(
          ...
          title: Row(
              ...
              children: <Widget>[
                new Text(
                  corona_data[i].name.toUpperCase().substring(0, corona_data[i].name.length > 18 ? 18 : corona_data[i].name.length),
  ...
  Widget build(BuildContext context) {
    if (_filter != null) {
      if (_filter == FILTERS.CASES) {
        ...
        _filtered_corona_data.sort((a, b) => a.deaths > b.deaths ? -1 : 1);
      } else {
        _filtered_corona_data
